...
export async function getTopCandidates({ limit = 10 } = {}) {
  ...
  if (eligible.length > 0) {
    ...
    if (uniqueMints.length < eligible.length) {
      log("okx", `Deduping OKX enrichment: ${eligible.length} pools → ${uniqueMints.length} unique mints`);
    }
    const okxResults = await Promise.allSettled(
      uniqueMints.map(async (mint) => {
        ...
        const names = eligible.filter((p) => p.base?.mint === mint).map((p) => p.name).filter(Boolean).join(", ") || mintShort;
        if (adv.status !== "fulfilled")      log("okx", `advanced-info unavailable for ${names} (${mintShort})`);
        if (price.status !== "fulfilled")    log("okx", `price-info unavailable for ${names} (${mintShort})`);
        if (clusters.status !== "fulfilled") log("okx", `cluster-list unavailable for ${names} (${mintShort})`);
        if (risk.status !== "fulfilled")     log("okx", `risk-check unavailable for ${names} (${mintShort})`);
    ...
    const okxByMint = new Map();
    for (const r of okxResults) {
      if (r.status === "fulfilled" && r.value?.mint) okxByMint.set(r.value.mint, r.value);
    }
    for (let i = 0; i < eligible.length; i++) {
      const { adv, price, clusters, risk } = okxByMint.get(eligible[i].base?.mint) || {};
      if (adv) {
